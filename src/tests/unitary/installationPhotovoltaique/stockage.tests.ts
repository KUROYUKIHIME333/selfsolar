...
describe("StockageService", () => {
  ...
  describe("capaciteStockage", () => {
    ...
    it("doit calculer correctement pour Lithium NMC/NCA", () => {
      ...
      expect(result.data?.cyclesDoDMax.min).toBe(500);
      expect(result.data?.cyclesDoDMax.max).toBe(2000);
      expect(result.data?.plageTemperatureFonctionnement.min).toBe(0);
      expect(result.data?.plageTemperatureFonctionnement.max).toBe(45);
    });
    ...
    it("doit calculer correctement pour NiCd", () => {
      ...
      expect(result.data?.cyclesDoDMax.min).toBe(1500);
      expect(result.data?.cyclesDoDMax.max).toBe(3500);
      expect(result.data?.plageTemperatureFonctionnement.min).toBe(-20);
      expect(result.data?.plageTemperatureFonctionnement.max).toBe(50);
    });
    ...
    it("doit gérer des valeurs décimales avec précision", () => {
      ...
      expect(result.data?.capacite.utile_Wh).toBe(1852); // round(1234.5 * 1.5)
      expect(result.data?.capacite.nominale_Wh).toBe(2315); // round(1852 / 0.8)
      expect(result.data?.capacite.nominale_Ah).toBeCloseTo(48.2, 1); // round(2315/48 * 10)/10
    });
  ...
  describe("modulesBatteries", () => {
    it("doit calculer la disposition pour un système 48V avec batteries 12V", () => {
      ...
      expect(result.data?.appareil).toBe("Batteries");
      expect(result.data?.disposition.batteriesParString).toBe(4); // 48/12 = 4
      expect(result.data?.disposition.modulesEnParallele).toBe(2); // ceil(200/100)
      expect(result.data?.nombre).toBe(8); // 4 * 2
    });

    it("doit calculer la disposition pour un système 24V avec batteries 6V", () => {
      const result = service.modulesBatteries(24, 6, 200, 400);

      expect(result.data?.disposition.batteriesParString).toBe(4); // 24/6 = 4
      expect(result.data?.disposition.modulesEnParallele).toBe(2); // ceil(400/200)
    ...
    it("doit calculer avec un seul string si capacité suffisante", () => {
      ...
      expect(result.data?.disposition.modulesEnParallele).toBe(1); // ceil(200/300) = 1
    ...
    it("doit arrondir batteriesParString à l'entier le plus proche", () => {
      ...
      expect(result.data?.disposition.batteriesParString).toBe(4); // round(48/12.8) = round(3.75) = 4
    });
    ...
    it("doit gérer des valeurs décimales", () => {
      ...
      expect(result.data?.disposition.batteriesParString).toBe(15); // round(48/3.2) = 15
      expect(result.data?.disposition.modulesEnParallele).toBe(2); // ceil(560/280)
  ...
  describe("regulateurBMS", () => {
    ...
    it("doit gérer un projet industriel de grande puissance", () => {
      ...
      expect(result.data?.IBMSRecommande).toBe(140); // 104.17*1.25=130.2 -> 140
    });
  });
  ...
  describe("Robustesse technique", () => {
    ...
    it("doit gérer des paramètres avec décimales", () => {
      ...
      expect(result.data?.capacite.utile_Wh).toBe(3086); // round(1234.56 * 2.5)
  ...
  describe("Tests d'intégration - Flux complet", () => {
    it("doit dimensionner un système de stockage complet 48V LiFePO4", () => {
      ...
      expect(modules.data?.disposition.batteriesParString).toBe(15); // 48/3.2
      expect(modules.data?.disposition.modulesEnParallele).toBe(4); // ceil(312.5/100)
    ...
    it("doit dimensionner un système 12V Plomb-acide pour site isolé", () => {
      ...
      expect(modules.data?.disposition.batteriesParString).toBe(2); // 12/6
      expect(modules.data?.disposition.modulesEnParallele).toBe(1); // ceil(83.3/100)

      const bms = service.regulateurBMS(300, 12, 250, 0.95);
      expect(bms.data?.IChargeMax).toBe(25);
      expect(bms.data?.IBMSRecommande).toBe(40); // max(25, 21.93)*1.25=31.25 -> 40
    });
  });
});
